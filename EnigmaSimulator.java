...
public class EnigmaSimulator {
    ...
    public EnigmaSimulator() throws IOException {
        ...
        // Inizializza la configurazione del plugboard prendendo il valore dal file di
        // configurazione
    ...
    private char applicaPlugboard(char c) {
        if (plug != null && !plug.isEmpty()) {
            // Se il plugboard è configurato, scambia i caratteri in base alla
            // configurazione
            for (String swap : plug.split(",")) {
                if (swap.charAt(0) == c)
                    return swap.charAt(2);
                if (swap.charAt(2) == c)
                    return swap.charAt(0);
            }
        }
        // Restituisce il carattere originale se non ci sono configurazioni nel
        // plugboard
        return c;
    }
    ...
    private char transforma(char c, String rotore, boolean cifra) {
        ...
        if (cifra) {
            ...
            // Se si sta decifrando, restituisce il carattere corrispondente nell'alfabeto
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length != 2) {
            // Se non ci sono due argomenti, stampa un messaggio di errore e termina il
            // programma
